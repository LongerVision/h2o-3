...
ice_plot_display_mode_grouping_variable <- function() {
  ...
  for (col in cols_to_test) {
    if (col != "name") { # a string column
       expect_ggplot(h2o.ice_plot(gbm, train, col, grouping_variable = "title")[[1]])
...
doSuite("Explanation Tests", makeSuite(
    ice_plot_display_mode_grouping_variable,
